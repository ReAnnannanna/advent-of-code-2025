...
fn collect_accessible_cells(grid: &Grid) -> Vec<(usize, usize)> {
    grid.iter()
        .enumerate()
        ...
        .map(|(row_index, column_index, _cell)| (row_index, column_index))
        .collect()
}
...
fn print_grid(grid: &Grid) {
    for row in grid {
        let line = row
            .iter()
            .map(|cell| if *cell == Cell::Paper { '@' } else { '.' })
            .collect::<String>();
        println!("{line}");
    }
    println!();
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut count = 0;
    loop {
        let cells = collect_accessible_cells(&grid);
        if cells.is_empty() {
            break;
        }
        for (row_index, column_index) in cells {
            grid[row_index][column_index] = Cell::Empty;
            count += 1;
        }
    }
